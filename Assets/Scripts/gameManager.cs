...
public class gameManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(!isPaused)
        {
            ...
            if(fastMode && Time.timeScale != 3.0f)
            {
                Time.timeScale = 3.0f;
            } else {
                Time.timeScale = 1.0f;
    ...
    */
    ...
    private IEnumerator waveManager ()
    {
        while (true)
        {
            ...
            if (waveNumber <= 21 && waveNumber % 3 == 0)
            {
                spawnDelay *= 0.5f;
                if (waveNumber > 3)
                {
                    scaleAmount = scaleAmount * (1 + scaleDecay);
                }
            }

            if (waveNumber <= 5)
            {
                money += (float)waveNumber * 200;
            }
    ...
    public void iBuyTower(string name)
    {
        ...
        switch (name)
        {
            case "laser":
                curTower = Towers[0];
                towImage.sprite = towerImages[0];
                break;
            case "aoe":
                curTower = Towers[1];
                towImage.sprite = towerImages[1];
                break;
            case "chill":
                curTower = Towers[2];
                towImage.sprite = towerImages[2];
                break;
            default:
                return;
        }
